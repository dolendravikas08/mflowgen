...
def makefile_floorplan():
    ...
    util.replace_line_in_file(makefile_path,
        ...
        '$(eval $(call do-step,2_1_floorplan,$(RESULTS_DIR)/floorplan_input.v $(RESULTS_DIR)/floorplan_input.sdc $(TECH_LEF) $(SC_LEF) $(ADDITIONAL_LEFS) $(FOOTPRINT) $(SIG_MAP_FILE) $(FOOTPRINT_TCL),floorplan))'
    )
    ...
    util.replace_line_in_file(makefile_path,
        ...
        '$(eval $(call do-step,2_3_floorplan_tdms,$(RESULTS_DIR)/2_2_floorplan_io.odb $(RESULTS_DIR)/floorplan_input.v $(RESULTS_DIR)/floorplan_input.sdc $(LIB_FILES),tdms_place))'
    )
    
    util.replace_line_in_file(makefile_path,
        '$(eval $(call do-copy,2_3_floorplan_tdms,2_2_floorplan_io.odb,$(RESULTS_DIR)/1_synth.v $(RESULTS_DIR)/1_synth.sdc $(LIB_FILES)))',
        '$(eval $(call do-copy,2_3_floorplan_tdms,2_2_floorplan_io.odb,$(RESULTS_DIR)/floorplan_input.v $(RESULTS_DIR)/floorplan_input.sdc $(LIB_FILES)))'
    )
    ...
    util.replace_line_in_file(makefile_path,
        ...
        '$(eval $(call do-step,2_4_floorplan_macro,$(RESULTS_DIR)/2_3_floorplan_tdms.odb $(RESULTS_DIR)/floorplan_input.v $(RESULTS_DIR)/floorplan_input.sdc $(MACRO_PLACEMENT) $(MACRO_PLACEMENT_TCL),macro_place))'
    )
    ...
    util.comment_out_and_add_line(makefile_path,
        ...
        '$(eval $(call do-copy,floorplan_output,2_5_floorplan_tapcell.odb,))'  
    )
...
def tcl_power():
    ...
    util.replace_line_in_file(pdn_tcl,
        ...
        'load_design power_input.odb power_input.sdc'
    )
    ## rename output file names
    util.replace_line_in_file(pdn_tcl,
        '  write_def $::env(RESULTS_DIR)/2p5_power.def',
        '  write_def $::env(RESULTS_DIR)/power_output.def'
    )

    util.replace_line_in_file(pdn_tcl,
        'write_db $::env(RESULTS_DIR)/2_6_floorplan_pdn.odb',
        'write_db $::env(RESULTS_DIR)/power_output.odb'
    )

def makefile_place():
    util.replace_line_in_file(makefile_path,
        'place: $(RESULTS_DIR)/3_place.odb \\',
        'place: $(RESULTS_DIR)/place_output.odb \\'  
    )

    util.replace_line_in_file(makefile_path,
        '       $(RESULTS_DIR)/3_place.sdc',
        '       $(RESULTS_DIR)/place_output.sdc'  
    )
    ...
    util.replace_line_in_file(makefile_path,
        ...
        '$(eval $(call do-step,3_1_place_gp_skip_io,$(RESULTS_DIR)/place_input.odb $(RESULTS_DIR)/place_input.sdc $(LIB_FILES),global_place_skip_io))'  
    )

    util.replace_line_in_file(makefile_path,
        '$(eval $(call do-step,3_3_place_gp,$(RESULTS_DIR)/3_2_place_iop.odb $(RESULTS_DIR)/2_floorplan.sdc $(LIB_FILES),global_place))',
        '$(eval $(call do-step,3_3_place_gp,$(RESULTS_DIR)/3_2_place_iop.odb $(RESULTS_DIR)/place_input.sdc $(LIB_FILES),global_place))'  
    )

    util.replace_line_in_file(makefile_path,
        '$(eval $(call do-step,3_4_place_resized,$(RESULTS_DIR)/3_3_place_gp.odb $(RESULTS_DIR)/2_floorplan.sdc,resize))',
        '$(eval $(call do-step,3_4_place_resized,$(RESULTS_DIR)/3_3_place_gp.odb $(RESULTS_DIR)/place_input.sdc,resize))'  
    )

    util.replace_line_in_file(makefile_path,
        '$(eval $(call do-copy,3_place,3_5_place_dp.odb,))',
        '$(eval $(call do-copy,place_output,3_5_place_dp.odb,))'  
    )

    util.replace_line_in_file(makefile_path,
        '$(eval $(call do-copy,3_place,2_floorplan.sdc,,.sdc))',
        '$(eval $(call do-copy,place_output,place_input.sdc,,.sdc))'  
    )
